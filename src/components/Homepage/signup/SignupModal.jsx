import React from "react";

export default function SignupModal({ open, onClose, onOtp }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-10">
      <div className="relative bg-white rounded-3xl shadow-lg w-[450px] max-w-full p-10 flex flex-col items-center">
        {/* Close button */}
        <button
          className="absolute top-6 right-6 text-2xl text-gray-700 hover:text-black"
          onClick={onClose}
          aria-label="Close"
        >
          &times;
        </button>
        {/* Header */}
        <h2 className="text-3xl font-bold mb-2 tracking-wide text-center">Sign up</h2>
        <div className="w-3/4 border-t border-black mb-8" />
        {/* Form */}
        <form className="w-full flex flex-col items-center gap-8" onSubmit={e => { e.preventDefault(); onOtp && onOtp(); }}>
          <div className="w-full">
            <label className="block text-xl mb-2 text-gray-800 font-serif">User Name</label>
            <input
              type="text"
              className="w-full rounded-2xl px-6 py-4 text-lg shadow-md focus:outline-none focus:ring-2 focus:ring-yellow-400"
              placeholder="Enter your name"
            />
          </div>
          <div className="w-full">
            <label className="block text-xl mb-2 text-gray-800 font-serif">Phone Number</label>
            <input
              type="tel"
              className="w-full rounded-2xl px-6 py-4 text-lg shadow-md focus:outline-none focus:ring-2 focus:ring-yellow-400"
              placeholder="Enter your phone number"
            />
          </div>
          <button
            type="submit"
            className="w-40 bg-yellow-400 text-black font-bold text-2xl rounded-full py-2 shadow-md hover:bg-yellow-500 transition mb-2"
          >
            OTP
          </button>
        </form>
        <div className="my-2 text-gray-500">or</div>
        <button className="flex items-center gap-2 border border-black rounded-lg px-6 py-2 hover:bg-gray-100 transition">
          <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="w-6 h-6" />
          <span className="text-lg">continue with Google</span>
        </button>
      </div>
    </div>
  );
}
