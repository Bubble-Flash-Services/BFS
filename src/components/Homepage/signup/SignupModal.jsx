  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-10">
      <div className="relative bg-white rounded-3xl shadow-lg w-full max-w-[400px]  max-h-screen overflow-y-auto p-6 sm:p-6 flex flex-col items-center justify-center">
        ...
        {/* Form */}
        <form className="w-full flex flex-col items-center gap-4 sm:gap-8" onSubmit={e => { e.preventDefault(); onSignup && onSignup(); }}>
        ...
        <button className="flex items-center gap-2 border border-black rounded-lg px-4 sm:px-6 py-2 hover:bg-gray-100 transition">
          <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="w-6 h-6" />
          <span className="text-base sm:text-lg">continue with Google</span>
        </button>
        <div className="text-center text-gray-600 text-sm mt-2">
          Have an account?{' '}
          <button className="text-blue-500 hover:underline" onClick={onLoginNow}>Log in now</button>
        </div>
      </div>
    </div>
  );
