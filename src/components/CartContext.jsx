...
export function CartProvider({ children }) {
  ...
  const deriveCategory = (rawItem) => {
    try {
      ...
      if (
        kind.includes("green") ||
        cat.includes("green") ||
        /green.*clean|home.*clean|sofa.*clean|carpet.*clean|bathroom.*clean|kitchen.*clean|office.*clean/i.test(
          svcName
        )
      ) {
        return "Green & Clean";
      ...
      if (kind.includes("accessory") || /accessor/.test(svcName))
      ...
      if (
        ...
        return "Bike Wash";
      if (
        ["car", "hatchback", "sedan", "suv", "luxury", "luxuries"].includes(
          vt
        ) ||
        ["suv", "sedan", "hatchbacks", "luxuries"].includes(cat)
      )
        return "Car Wash";
      if (
        /(laundry|wash & fold|dry clean|ironing)/i.test(
          rawItem.serviceId?.name || rawItem.name || ""
        )
      )
  ...
  useEffect(() => {
    return () => {
      console.log(
        "🗑️ CartProvider unmounting for user:",
        user?.email || "guest"
      );
      console.log(
        "🛒 Cart items at unmount:",
      ...
      if (
        ...
        const formattedItems = items.map((item) => ({
          ...
          img:
            ...
            getDefaultServiceImage(item.serviceName || item.name),
          image:
            item.image ||
            item.img ||
            item.serviceId?.image ||
        ...
        if (localCart) {
          try {
            ...
            if (localCartItems.length > 0) {
              ...
              const syncResponse = await cartApi.syncCartToDatabase(
                token,
                localCartItems
              );
    ...
    if (guestCart && user) {
      try {
        ...
        if (guestItems.length > 0) {
          ...
          console.log(
            "No guest cart items to migrate, starting with empty cart"
          );
          setCartItems([]);
        }
      } catch (error) {
        ...
        setCartItems(
          items.map((item) => {
            ...
            return {
              ...
              serviceName:
                item.serviceName || item.name || item.serviceId?.name,
              ...
              includedFeatures:
                item.includedFeatures || derivedPackageDetails?.features || [],
              vehicleType: item.vehicleType,
              specialInstructions: item.specialInstructions,
  ...
  useEffect(() => {
    ...
    if (cartItems.length > 0) {
      if (user) {
        ...
        console.log(
          "🔍 Verification - saved data retrieved:",
  ...
  const addToCart = async (product) => {
    ...
    if (user && token) {
      ...
      try {
        ...
        const onlyValidObjectId = (v) =>
          typeof v === "string" && /^[0-9a-fA-F]{24}$/.test(v) ? v : undefined;
        const cartData = {
          serviceId: product.serviceId || product.id || product._id,
          serviceName: product.serviceName || product.name || product.title, // Prefer explicit serviceName
          ...
          includedFeatures:
            product.includedFeatures || product.packageDetails?.features || [],
  ...
  const updateQuantity = async (productId, newQuantity) => {
    ...
    if (user && token) {
      ...
      setLoading(true);
      try {
        const response = await cartApi.updateCartItem(token, productId, {
