...
const members = [
  {
    ...
    name: 'Nagaraj C',
    shortTagline: '"Transforming car & laundry services into a smarter experience."',
    ...
    vision: 'Position BFS as the most trusted premium brand blending convenience with quality.',
  ...
  },
  {
    ...
    vision: 'Create a motivated, efficient, professional workforce that differentiates BFS.',
    tagline: 'People first, quality always.',
    placeholder: 'HR'
  },
  {
    roleLabel: 'Full Stack Developer - Gowtham',
    name: 'Gowtham J',
    shortTagline: '"Engineering seamless digital solutions for Bubble Flash Services."',
    title: 'Full Stack Developer',
  image: '/photos/Gowtham.JPG',
    profile:
    ...
    vision: 'Empower BFS with scalable, modern platform that grows effortlessly.',
    tagline: 'Engineering seamless digital solutions for modern service businesses.',
    placeholder: 'DEV'
  }
];
...
export default function TeamPage() {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-white via-blue-50 to-white overflow-hidden">
      <div className="absolute inset-0 pointer-events-none opacity-40" style={{backgroundImage:'radial-gradient(circle at 20% 30%, rgba(255,215,0,0.18), transparent 60%), radial-gradient(circle at 80% 70%, rgba(59,130,246,0.18), transparent 55%)'}} />
      <div className="max-w-6xl mx-auto px-4 pb-24 pt-16 relative">
        ...
        <p className="mt-4 text-center text-gray-600 max-w-2xl mx-auto">The minds and hearts building Bubble Flash Services.</p>
            ...
            >
              ...
              <div className="flex flex-col md:flex-row gap-10">
                <div className="w-full md:w-64 flex flex-col items-center text-center md:text-left">
                  {m.image || m.placeholder ? <TeamImage src={m.image} alt={m.name} placeholder={m.placeholder} /> : null}
                  <h2 className="mt-6 text-2xl font-bold text-gray-800 leading-tight">{m.name}</h2>
                  <p className="text-sm uppercase tracking-wide font-semibold text-blue-600 mt-1">{m.roleLabel}</p>
                  <p className="mt-4 text-sm italic text-gray-600">{m.shortTagline}</p>
                </div>
                <div className="flex-1 space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">Title</h3>
                    <p className="text-gray-700 mt-1 leading-relaxed">{m.title}</p>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">Profile</h3>
                    <p className="text-gray-700 mt-1 leading-relaxed">{m.profile}</p>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-8">
                    <div>
                      <h4 className="font-semibold text-gray-900">Core Focus Areas</h4>
                      <ul className="mt-2 space-y-1 text-gray-700 text-sm list-disc list-inside">
                        {m.core.map(c => <li key={c}>{c}</li>)}
                      </ul>
                    </div>
                    <div>
