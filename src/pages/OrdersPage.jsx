...
export default function OrdersPage() {
  ...
  const handleDownloadInvoice = (order) => {
    ...
    const customerPhone =
      user?.phone || order.userId?.phone || phoneFromNotes || "";
      ...
      .flatMap((gk) => {
        ...
        <tr>
          ...
          <td>
            ...
            <div style="color:#555">${
              item.packageName || "Custom Package"
            }</div>
            ${
              Array.isArray(item.includedFeatures) &&
              item.includedFeatures.length
                ? `<div style="margin-top:6px"><em>Included:</em><ul>${item.includedFeatures
                    .map((f) => `<li>${f}</li>`)
                    .join("")}</ul></div>`
                : ""
            }
                ...
                ? [
                    ...
                    .map(([label, arr]) =>
                      Array.isArray(arr) && arr.length
                        ? `<div style=\"margin-top:4px\"><em>${label}:</em><ul>${arr
                            .map((x) => `<li>${x}</li>`)
                            .join("")}</ul></div>`
                        : ""
                    )
    ...
    ${
      order.discountAmount
        ? `<div><span>Discount${
          ...
          }</span><span>-₹${order.discountAmount}</span></div>`
        : ""
    }
    ${(() => {
      ...
      return (
        `<div><span>Taxable</span><span>₹${taxable}</span></div>` +
        `<div><span>CGST (9%)</span><span>₹${cgst}</span></div>` +
        `<div><span>SGST (9%)</span><span>₹${sgst}</span></div>`
      );
    })()}
    <div style="font-weight:700;border-top:1px solid #e5e7eb;margin-top:8px;padding-top:8px"><span>Total</span><span>₹${
      order.totalAmount
    }</span></div>
  </div>
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        // Update in list
        setOrders((prev) =>
          prev.map((o) => (o._id === reviewTarget._id ? result.data : o))
        );
        if (selectedOrder && selectedOrder._id === reviewTarget._id)
          setSelectedOrder(result.data);
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    >
                      {(selectedOrder.orderStatus || "pending").replace(
                        "_",
                        " "
                      )}
                    </span>
                      ...
                      <div>
                        ...
                        {selectedOrder.serviceAddress.city && (
                          <p>
                            ...
                            {selectedOrder.serviceAddress.pincode}
                          </p>
                        )}
                        {selectedOrder.serviceAddress.landmark && (
                          <p className="text-sm text-gray-600">
                            Landmark: {selectedOrder.serviceAddress.landmark}
                          </p>
                        )}
                      </div>
                    ) : (
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                ...
                {(selectedOrder.orderStatus === "completed" ||
                  ...
                  !selectedOrder.isReviewSubmitted && (
                    <button
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="mt-3 p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded">
                          ...
                          {Array.isArray(item.includedFeatures) &&
                            item.includedFeatures.length > 0 && (
                              <div className="mt-2">
                          ...
                          {item.planDetails && (
                            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                              {Array.isArray(item.planDetails.washIncludes) &&
                                item.planDetails.washIncludes.length > 0 && (
                                  <div className="bg-white rounded p-3 border border-blue-200">
                                    <p className="text-xs font-semibold text-gray-700 mb-1">
                                      Each Wash Includes
                                    </p>
                                    <ul className="list-disc ml-5 text-xs text-gray-600 space-y-1">
                                      {item.planDetails.washIncludes.map(
                                        (f, i) => (
                                          <li key={i}>{f}</li>
                                        )
                                      )}
                              ...
                              {Array.isArray(item.planDetails.weeklyIncludes) &&
                                item.planDetails.weeklyIncludes.length > 0 && (
                                  <div className="bg-white rounded p-3 border border-blue-200">
                                    <p className="text-xs font-semibold text-gray-700 mb-1">
                                      Weekly Includes
                                    </p>
                                    <ul className="list-disc ml-5 text-xs text-gray-600 space-y-1">
                                      {item.planDetails.weeklyIncludes.map(
                                        (f, i) => (
                                          <li key={i}>{f}</li>
                                        )
                                      )}
                              ...
                              {Array.isArray(
                                item.planDetails.biWeeklyIncludes
                              ) &&
                                item.planDetails.biWeeklyIncludes.length >
                                  0 && (
                                  <div className="bg-white rounded p-3 border border-blue-200">
                                    <p className="text-xs font-semibold text-gray-700 mb-1">
                                      Bi-Weekly Includes
                                    ...
                                    <ul className="list-disc ml-5 text-xs text-gray-600 space-y-1">
                                      {item.planDetails.biWeeklyIncludes.map(
                                        (f, i) => (
                                          <li key={i}>{f}</li>
                                        )
                                      )}
                              ...
                              {Array.isArray(item.planDetails.monthlyBonuses) &&
                                item.planDetails.monthlyBonuses.length > 0 && (
                                  <div className="bg-white rounded p-3 border border-blue-200">
                                    <p className="text-xs font-semibold text-gray-700 mb-1">
                                      Monthly Bonuses
                                    </p>
                                    <ul className="list-disc ml-5 text-xs text-gray-600 space-y-1">
                                      {item.planDetails.monthlyBonuses.map(
                                        (f, i) => (
                                          <li key={i}>{f}</li>
                                        )
                                      )}
                              ...
                              {Array.isArray(item.planDetails.platinumExtras) &&
                                item.planDetails.platinumExtras.length > 0 && (
                                  <div className="bg-white rounded p-3 border border-blue-200">
                                    <p className="text-xs font-semibold text-gray-700 mb-1">
                                      Premium Extras
                                    </p>
                                    <ul className="list-disc ml-5 text-xs text-gray-600 space-y-1">
                                      {item.planDetails.platinumExtras.map(
                                        (f, i) => (
                                          <li key={i}>{f}</li>
                                        )
                                      )}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center gap-2 mb-3">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    className={
                      "p-1 " +
                      (reviewRating >= n ? "text-yellow-500" : "text-gray-300")
                    }
                  >
                    <Star
                      size={20}
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600">
              Track your service requests and order history
            </p>
          </div>
          <button
            onClick={fetchOrders}
